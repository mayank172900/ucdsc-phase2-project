#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# This is the main code used for synthetic experiments.
# It produces 2-dimensional CNN embeddings.
"""
...
from modules.NirvanaLoss import center_loss_nirvana, nirvana_mics_loss, cross_entropy_nirvana, accuracy_l2_nosubcenter, get_l2_pred_nosubcenter, cosine_similarity, euc_cos, NirvanaHinge
...
def train_one_epoch(model,centerloss, optimizer, data_loader, device, epoch, args):
    ...
    # plotting 2-dimesional CNN features
    ax = plot_features(all_features, all_labels)
    # create a folder "figures_default_simplex_loss" in advance
    utils.mkdir('/home/mlcv/CevikalpPy/nirvana/figures_simplex_softmax_loss')
    ax.figure.savefig(os.path.join("figures_simplex_softmax_loss","fig_new_%d.png"%epoch))
    plt.close('all')
...
def main(args):
    save_dir = os.path.join('logs','nirvana_CIFAR10','lr%.6f_nirvana_Expand%d_Epoch%d_Seed%d_Wd%.f'%(args.lr, args.Expand, args.epochs, args.Seed, args.weight_decay))
    ...
    dataset = CIFAR10_OSR(known = [0,1,2],batch_size = args.batch_size, use_gpu = True) # selecting the first 3 classes from cifar-10 for visualization
    ...
    model = LeNet(args.num_classes) # this is a small network that yields 2-dimensional CNN features 
    model.to(device)
    ...
    # default dsc loss
    centerloss = cross_entropy_nirvana(args.num_classes,args.feat_dim,True,device, Expand=args.Expand)
...
def parse_args():
    ...
    parser.add_argument('--Expand', default=5, type=int, metavar='N',
