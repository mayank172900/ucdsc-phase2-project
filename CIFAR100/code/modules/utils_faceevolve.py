...
def perform_val(device, embedding_size, batch_size, backbone, carray, issame, nrof_folds = 10, tta = False):
    ...
    with torch.no_grad():
        while idx + batch_size <= len(carray):
            ...
            if tta:
                ...
                ccropped = ccrop_batch(batch)
                embeddings[idx:idx + batch_size] = l2_norm(backbone(ccropped.to(device))[0]).cpu()
            idx += batch_size
        if idx < len(carray):
            ...
            if tta:
                ...
                ccropped = ccrop_batch(batch)
                embeddings[idx:] = l2_norm(backbone(ccropped.to(device))[0]).cpu()
