...
from datasets.osr_dataloader import Random300K_Images, BloodMNIST_OSR, OCTMnist_OSR, DermaMNIST_OSR, TissueMNIST_OSR, PathMNIST_OSR, ASC_OSR
...
class ProjectedModel(nn.Module):
    def __init__(self, backbone: nn.Module, in_dim: int, embed_dim: int, num_classes: int) -> None:
        super().__init__()
        self.backbone = backbone
        self.project = nn.Linear(in_dim, embed_dim, bias=False)
    ...
    def forward(self, x, return_feature=False):
        features, _ = self.backbone(x, True)
...
# Dataset
parser.add_argument('--dataset', type=str, default='bloodmnist', choices=['bloodmnist', 'octmnist', 'dermamnist', 'tissuemnist', 'pathmnist', 'asc'], help="Dataset selection")
...
parser.add_argument('--embed-dim', type=int, default=None, help='Optional projected embedding dimension used before the simplex loss')
...
def main_worker(options):
    ...
    elif options['dataset'] == 'pathmnist':
        split_dict = splits[options['dataset']][options['item']]
        known = split_dict['known']
        unknown = split_dict['unknown']
        Data = PathMNIST_OSR(
    ...
    requested_embed_dim = options.get('embed_dim')
    model_tag = options['model']
    if requested_embed_dim is not None and requested_embed_dim != feat_dim:
        if requested_embed_dim <= 0:
            raise ValueError("--embed-dim must be a positive integer")
        print(f"Applying projection head: {feat_dim} -> {requested_embed_dim}")
        net = ProjectedModel(net, feat_dim, requested_embed_dim, options['num_classes'])
        feat_dim = requested_embed_dim
        model_tag = f"{options['model']}_ed{feat_dim}"
    ...
    use_precalc_centers = options['feat_dim'] >= (options['num_classes'] - 1)
    if not use_precalc_centers:
        print(
            f"Skipping simplex center initialization because "
            f"feat_dim={options['feat_dim']} < num_classes-1={options['num_classes'] - 1}"
        )

    criterion = NirvanaOpenset_loss(
            num_classes=options['num_classes'], 
            feat_dim=options['feat_dim'], 
            precalc_centers=use_precalc_centers, 
    ...
    dir_name = '{}_{}_{}_{}'.format(options['model_tag'], options['loss'],options['margin'],options['oe'])
    model_path = os.path.join(options['outf'], 'models', options['dataset'],dir_name)
    if not os.path.exists(model_path):
        os.makedirs(model_path)
    
    file_name = '{}_{}_{}_{}_{}'.format(options['model_tag'], options['loss'], options['item'], options['margin'],options['noisy_ratio'])
...
if __name__ == '__main__':
    ...
    options = vars(args)
    options['model_tag'] = (
        f"{options['model']}_ed{options['embed_dim']}"
        if options.get('embed_dim') is not None
        else options['model']
    )
    ...
    for i in split_indices:
        ...
        dir_name = '{}_{}_{}_{}_{}'.format(options['model_tag'], options['loss'],options['margin'],options['oe'],options['noisy_ratio'])
