...
def metric_ood(x1, x2, stypes = ['Bas'], verbose=True):
    ...
    for stype in stypes:
        ...
        results[stype][mtype] = 100.*(-_trapz(1.-fpr, tpr))
        if verbose:
            print(' {val:6.3f}'.format(val=results[stype][mtype]), end='')
        ...
        results[stype][mtype] = 100.*(-_trapz(pin[pin_ind], tpr[pin_ind]))
        if verbose:
            print(' {val:6.3f}'.format(val=results[stype][mtype]), end='')
        ...
        results[stype][mtype] = 100.*(_trapz(pout[pout_ind], 1.-fpr[pout_ind]))
        if verbose:
            print(' {val:6.3f}'.format(val=results[stype][mtype]), end='')
            print('')
